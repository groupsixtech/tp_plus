      ...
      def string_val(context, options={})
        if @op.bang?
          ...
          if @op.boolean?
            ...
            "#{@left_op.eval(context, opposite: (options[:eval] != false && options[:opposite]))}#{@op.eval(context, options)}#{@right_op.eval(context, options)}"
          else
            # flip the operator if options[:opposite]
            # flip the operands only if opposite and the operand is an expression
            "#{@left_op.eval(context, opposite: (@left_op.is_a?(ExpressionNode) && options[:opposite]))}#{@op.eval(context, options)}#{@right_op.eval(context, opposite: (@right_op.is_a?(ExpressionNode) && options[:opposite]), eval: (@op.boolean?))}"
          end
        end
      end
